// Pantallas narrativas entre minijuegos. Texto puente con tono de historia.
(function () {
  'use strict';

  const W = 360, H = 640;

  const SCRIPTS = {
    NARRATIVE_1: {
      next: 'INVADERS',
      title: 'CAPÍTULO I',
      lines: [
        'Pedrito Skywalker surca',
        'el espacio rumbo a la',
        'Estrella de Kike. De pronto,',
        'una flota de cazas TIE le',
        'cierra el paso.',
        '',
        'Solo su nave y sus reflejos',
        'pueden abrirle camino.',
      ],
    },
    NARRATIVE_2: {
      next: 'RACING',
      title: 'CAPÍTULO II',
      lines: [
        'Tras la batalla, Pedrito',
        'Skywalker aterriza en el',
        'gran almacén imperial de',
        'materiales de construcción.',
        '',
        'Su Manitou Milenaria debe',
        'esquivar palets, agujeros,',
        'ladrillos y sacos de cemento',
        'para llegar al fondo.',
      ],
    },
    NARRATIVE_3: {
      next: 'COMPACTOR',
      title: 'CAPÍTULO III',
      lines: [
        'Pedrito Skywalker, en su',
        'persecución, ha caído en la',
        'compactadora de basura del',
        'centro de residuos.',
        '',
        'Tendrá que utilizar su ingenio',
        'para poder salir de ella antes',
        'de que le aplaste.',
      ],
    },
    NARRATIVE_4: {
      next: 'LIGHTSABER',
      title: 'CAPÍTULO IV',
      lines: [
        'Por fin, frente a frente.',
        'Kike Vader y su sable rojo.',
        'La Princesa Marian observa,',
        'atrapada.',
        '',
        'Si Pedrito Skywalker pierde,',
        'ella será besada por el villano.',
        '',
        'Que la Fuerza le acompañe.',
      ],
    },
  };

  function createNarrative(key) {
    let timer, alpha;
    let stars;

    function enter() {
      timer = 0;
      alpha = 0;
      stars = window.Stars.createField({ width: W, height: H, count: 50, speed: 10 });
    }

    function update(dt) {
      timer += dt;
      alpha = Math.min(1, timer / 0.8);
      stars.update(dt);

      if (timer > 1.0 && window.Input.actionJustPressed()) {
        window.Loop.setScene(SCRIPTS[key].next);
      }
    }

    function render(ctx) {
      ctx.fillStyle = '#000';
      ctx.fillRect(0, 0, W, H);
      stars.render(ctx);

      const cfg = SCRIPTS[key];

      ctx.save();
      ctx.globalAlpha = alpha;

      ctx.fillStyle = '#ffe81f';
      ctx.font = '14px "Press Start 2P", monospace';
      ctx.textAlign = 'center';
      ctx.fillText(cfg.title, W / 2, 80);

      ctx.font = '10px "Press Start 2P", monospace';
      ctx.fillStyle = '#fff';
      for (let i = 0; i < cfg.lines.length; i++) {
        ctx.fillText(cfg.lines[i], W / 2, 140 + i * 22);
      }

      // Llamada a la acción parpadeante
      if (Math.floor(timer * 2) % 2 === 0 && timer > 1.0) {
        ctx.fillStyle = '#ffe81f';
        ctx.font = '8px "Press Start 2P", monospace';
        ctx.fillText('TOCA O PULSA ENTER', W / 2, H - 60);
      }

      // HUD de vidas
      drawLives(ctx);

      ctx.restore();
    }

    return { enter, update, render };
  }

  function drawLives(ctx, x, y) {
    const ox = (typeof x === 'number') ? x : 10;
    const oy = (typeof y === 'number') ? y : 12;
    const s = window.GameState.state;
    for (let i = 0; i < s.maxLives; i++) {
      window.Characters.drawHeart(ctx, ox + i * 12, oy, 1.5, i < s.lives);
    }
    if (s.infiniteLives) {
      ctx.fillStyle = '#ffe81f';
      ctx.font = '8px "Press Start 2P", monospace';
      ctx.fillText('∞', ox + s.maxLives * 12 + 2, oy + 12);
    }
  }

  Object.keys(SCRIPTS).forEach(key => {
    window.Loop.register(key, createNarrative(key));
  });

  window.NarrativeHUD = { drawLives };
})();
